...
class RobotController(Node):
    def __init__(self):
        ...
        self.position_map = {
            ...
            "pos2":[565.341, -212.003, 191.381, 120.138, -144.982,-149.638], # 불량품 바구니
        }
    ...
    #     basket_up = [456.52,-149.709,292.114,129.739,-164.505,-120.831] # put_pos에서 z값만 높힌 위치 (양품과 불량품 놓을 basket 중간 지점)
    ...
    def robot_control(self):
        ...
        if get_keyword_future.result().success:
            ...
            if "/" in message:
                object_str, target_str = message.split("/") ##num_str
            ...
            if object_list[0] == 'pimang'  and len(target_list) == 2:
                while self.section_num < 5:
                    ...
                    if target_pos is None:
                        ...
                        mwait()
                        self.pick_to_put_place_n(self.position_map[target_list[0]])
                        self.sterilize()
            ...
            if object_list[0] == 'rotten'  and target_list[1] == 'pos2':
                while self.section_num < 5:
                    ...
                    if self.state == 'failed': 
                        ...
                        self.state = 'normal'
                        if self.section_num >= 4:
                            self.get_logger().warn("섹션 모두 탐색 완료")
                            break
                    mwait()
                    movel(self.scan_section[self.section_num], vel=VELOCITY, acc=ACC)
...
##################양품 지정 개수 따기###########################################################미구현
    ...
    def get_target_pos(self, target):
        try:
            ...
            self.get_logger().warn('get_target_pos time out or error')
            return None
